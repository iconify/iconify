...
describe('Redundancy class', () => {
	it('Simple query', () => {
		return new Promise((fulfill) => {
			...
			// Test find()
			expect(redundancy.find((item) => (item().payload as string) === '/foo')).toEqual(query);
			expect(redundancy.find((item) => item().status === 'pending')).toEqual(query);
		});
	});
	...
	it('Different start index', () => {
		return new Promise((fulfill) => {
			const redundancy = initRedundancy({
				resources: ['https://api.local', 'https://api-backup1.local', 'https://api-backup2.local'],
				rotate: 20,
				timeout: 3000,
				index: 1,
			});
			...
			const query = redundancy.query('/foo', (resource, payload, callback) => {
				counter++;
			...
			// Test find()
			expect(redundancy.find((item) => item().payload === '/foo')).toEqual(query);
			expect(redundancy.find((item) => item().status === 'pending')).toEqual(query);
		});
	});
});
